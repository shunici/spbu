               ...
               <b-table striped hover responsive  bordered :items="pemasukans" small
                ...
                <template #row-details="row">
                    <div :key="'detail' + row.index" class="card-body bg-white" >                                       
                            <img   @error="event => event.target.style.display = 'none'" class="img-fluid" :src="'/storage/pemasukan/'+row.item.foto">
                            <img  @error="event => event.target.style.display = 'none'"  class="img-fluid" :src="'/storage/pemasukan/'+row.item.foto1">
                            <img  @error="event => event.target.style.display = 'none'"  class="img-fluid" :src="'/storage/pemasukan/'+row.item.foto2">
                            <img  @error="event => event.target.style.display = 'none'" class="img-fluid" :src="'/storage/pemasukan/'+row.item.foto3">
                                <hr>
