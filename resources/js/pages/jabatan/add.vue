<template>  
 <b-modal size="xl" id="add-jabatan" title="Buat Jabatan Baru"  :header-bg-variant="'info'">
 ...
 <div class="form-group">
        <label for="gajih_pokok" class="text-uppercase">gajih pokok</label>
  ...
  <template #modal-footer>
        <div class="w-100">         
          ...
          >                  
            ...
            size="sm"
            class="float-left"
            @click="$bvModal.hide('add-jabatan')"
          >
            Tutup
          </b-button>
        </div>
      ...
      export default {
          ...
          methods : {
              ...mapMutations('jabatan_stores', ['CLEAR_FORM']),    
                ...mapActions('jabatan_stores', ['submit_jabatan', 'get_jabatan']),            
              proses (ev) {
                  ev.preventDefault()
                  this.submit_jabatan().then(() => {
                        this.pesan.sukses = true
                     setTimeout(function () {
                        this.get_jabatan();
                        //close modal
                        this.$bvModal.hide('add-jabatan')
                        this.CLEAR_FORM();
