<template>
  ...
  <div class="card">
      <div class="card-header">
        <div class="card-title"><h3>Kategori Pengeluaran</h3></div>       
      ...
      <div class="card-footer">
              <router-link class="float-right" :to="{name : 'data.rekapitulasi'}">Detill Pengeluaran</router-link>
      </div>
  </div>
  ...
  methods: {
    async fetchData() {
      try {                    
        ...
        const pengeluaranData = response.data.data;        
        // Menyiapkan data untuk chart
        const labels = Object.keys(pengeluaranData.pengeluaran); // Nama kategori
        const data = Object.values(pengeluaranData.pengeluaran); // Total per kategori
        this.datacollection.labels = labels;
        this.datacollection.datasets = [
          {
            label: 'Pengeluaran ' + moment(this.bulan).format('MMMM'),
