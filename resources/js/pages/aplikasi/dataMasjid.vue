...
<div class="col-md-12">
    <label for="des" class="text-uppercase">deskripsi aplikasi</label>
       <textarea class="form-control" name="" id="des" cols="30" rows="10" v-model="aplikasi.tentang"></textarea>
</div>
         ...
         name : 'form-dataaplikasi',
          ...
          methods : {
           ...
           async deleteImage(imageSrc) {
             var nama_foto = imageSrc.split('/').pop(); // ambil nama filenya aja
            try {
              const response = await axios.post("api/delete_image", { src: nama_foto });
              console.info("Gambar berhasil dihapus:", response.data);
            } catch (error) {
              console.error("Gagal menghapus gambar:", error.response ? error.response.data : error);
            }
          },                                       
                add_sosmed(){
